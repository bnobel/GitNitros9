...
**            - Changed <gr00A9,u vector to use ,W instead (<>2 color txt on gfx
...
**            - Took out redundant LDB <Gr.STYMk @ L13E3 (clear to end of screen)
...
**   10/08/93 - Changed L1E2C to use LEAX B,U (since B always <gr0080) since
...
**            - L1DAA: eliminated LDD <gr0047 & changed CMPD <gr004B to CMPW <gr004B
...
**              shorter)
**            - Added LDU <gr0064 in L0E97, changed JSR [$>1164] in L0EE1 to
...
**   04/14/94 - Changed CMPB >$FFAC to CMPB <gr0087+9 (saves 1 byte/cycle & poss-
...
**              to <gr00A9 (since now free) to speed up/shrink checks.
...
**            - Attempted mod @ L05C0: Changed 1st TST <Gr.STYMk to LDE <Gr.STYMk, and
...
**            - Changed pixel across counter from <gr0097 to E reg in Gfx text
...
**            - Changed TST <Gr.STYMk @ L1260 to LDB <Gr.STYMk (saves 1 cycle)
...
**   10/22/94 - Eliminated useles LDB <Gr.STYMk @ L029B
...
**            - Changed TST <Gr.STYMk to LDB <Gr.STYMk @ L12C5, changed BRA L128E @
...
**   02/14/96 - added 'ldu <gr0064' U=pset vector to i.line, bar/box. -6 bytes,
...
** 09/25/03 - Many changes for 6809 only code. Use <gr00B5 to store regW
...
* Initialize DAT image
         ...
         std   <gr0087+2           Save it in rest 
* NOTE: IF 16K GRFDRV DONE,CHANGE FOLLOWING LINE TO STD <gr0087+8
...
* NOTE: IF 16K GRFDRV DONE,WILL HAVE TO COPY 4 (NOT 2) BYTES OF DAT IMAGE
         ...
         std   <gr0087+4           Save it to new task
         ELSE
         leay   a,y
         ldd   ,y++
         std   <gr0087+4
...
* ATD: changed from $1C98 for more lee-way on the stack
         ...
         std   <gr003B           Save in GRFDRV mem
         IFNE  H6309
         clrd                 Get screen table initialization
         clrw                 (CLRQ)
         stq   <gr002E           Init current screen table ptr & window entry
         stq   <gr003D           Init X/Y coords Gfx cursor was last ON at
         ELSE
         clra
         clrb
         std   <gr002E
...
*         std   <gr00B5
         ENDC
         stb   <gr0032           Clear out block #'s for G/P buffer (Current,
         stb   <gr0035            previous)
         std   <gr0039           Text cursor & gfx cursors off
L0102    clra
...
L0104    clr   <gr0038         Clear group #
         ...
         ldb   <gr0032         Get last block used
...
L0129    clr   <gr00A9         Special entry pt for DWSet,Select,UpdtWin,PutGC
L012B    ldx   Wt.STbl,y    Get screen table ptr
         ...
         std   <gr0061         Save it for this window
         IFNE  H6309
...
*         lda   #25          get maximum screen size in A
         lda   #60          get maximum screen size in A
...
* 30/08/2016 BN Nanomate has 6 screen sizes to check
* start at lowest
...
L02BB    lda   #$FF         preset counter
         sta   <gr00B3         unused grfdrv space
         ldy   #GrfStrt+L02FA-1   Point to RAM block table 
         andb  #$F          make it fit table
         ldb   b,y          get # blocks needed
         stb   <gr00B4         save number of blocks, unused space
OVLAP    inc   <gr00B3         update counter, unused space
...
DeMost   tst   <gr00B3         if none then return
         beq   DA020
         lda   <gr00B3
         pshs  a,x
...
L02FA    fcb   2            640 2 color
         ...
         fcb   4            640 4 color
         fcb   4            320 16 color
* 30/08/2016 BN Changed to 2 blocks for NanoMate double height
...
L0335    pshs  x,y          save regs
         ...
         std   <gr0097       init screen clear value to $00 (or attribute)
...
* Clear text screen
         ldx   <gr0097       get screen clear codes ($2000)
         IFNE  H6309
...
*         ldq   #(GrfStrt+L1FA9)*65536+(GrfStrt+L1F9E) Normal LSET/PSET vector
         ...
         stb   <gr0062       Store bckground bit mask in GRFDRV's global mem
...
* Exit:  <gr004F=X screen size (chars if hware text, pixels if Gfx)
*        <gr0051=Y screen size (char lines if hware text, pixels if Gfx)
L0516    pshs  x              Preserve root window table ptr
         ...
         ENDC
         clrb  
         std   <gr0047         Set current X coordinate to 0
...
L0611    ldb   <gr0057       get group mem block #
...
L0726    ldb   [Wt.STbl,y]   Get screen type from screen table
         stb   <Gr.STYMk        save it in global
...
L0748    clr   <gr00A9         No error, clear flag & return to system
         jmp   >GrfStrt+SysRet
...
L07E1    ldb   >WGlobal+G.CurTik       Reload counter for # ticks/cursor updates
         ...
         sty   <gr002E       save it as current window entry
         stx   <gr0030       set current screen table pointer
...
*        2 Meg of RAM)
         ...
         rola
         ENDC
         stb   <gr0082     Remainder is the block #(0-3F) in this 512k bank
...
* 30/08/2016 BN Mod for NanoMate 60 row
         ...
         cmpb  #25
         ble   no60
* enable double height on Nanomate for modes above 25 lines
         ldb   >$FF10
         orb   #$20
         stb   >$FF10
         bra   no61
...
no61     ldb   St.Brdr,y    Get current border palette #
...
L0949    lda   <gr0057       get group we're looking for
...
L09A8    ldd   <gr0080         Get original size we wanted
         ...
         ELSE
         stb   <gr00B5
         ldb   <gr00B6         regB does not need to be preserved
         cmpb  #1
         ENDC
...
L0ACF    lbsr  L096E        initialize previous buffer ptrs to 0 ($7D-$7F)
         ldb   <gr0035         get last buffer block #
         beq   L0B35        If 0, exit with carry set
         ldx   <gr0036         get offset of last one into 8K block
...
L0B22    ldb   <gr0087+3         Get GrfDrv MMU block # for get/put buffer block
...
* Move data between 2 blocks of memory
         ...
         ldd   <gr00B5
         subd  ,s           subr d,w
         std   <gr00B5         this is the value of regW after tfm & tfr d,w
...
* GetBlk entry point
L0BAE    lbsr  L1DF6          Go scale X/Y coords @ <gr0047-$4A,check if in range
         ...
         std   <gr004F
         ENDC
         lbsr  L1E01          Go scale X/Y Sizes @ <gr004F-$52,check if in range 
         bcs   L0BE7          No, exit with error
         IFNE  H6309
         ldq   <gr004F           Get X/Y sizes
         incd                 Bump back up
...
* Attempt reversing roles of D & W
         ...
         ldx   <gr0072       get address of screen
...
* <gr0057=Group #
* <gr0058=Buffer #
* <gr0047=Upper left X coord
* <gr0049=Upper left Y coord
L0CBB    lbsr  L0177        Go map in window & setup some GRFDRV vars
         ...
         stb   <gr0074       Save start pixel mask
         ldy   <gr007E       get ptr to GP buffer
...
GdTyp    tstb               graphics window?
         ...
         stb   <gr0000       Set right based pixel mask to all
         stb   <gr0001       Ditto for left based
...
*  full speed as well.
L0D27    bsr   L0D94        set up <gr004F+1 = X-count, <gr0051+1 = y-count
...
L0D3F    ldb   <Gr.STYMk       get screen type
         ...
         sta   <gr0000       Save right-based mask
         ldd   Grf.RtPx,y   Get right based pixel mask & GP buffer type
         bsr   L0D66        Go get starting/ending pixel masks
         stb   <gr0001       Save left-based pixel mask
...
L0D58    bsr   L0D94        Move x-size to $50 y-size to $52
         ...
         stb   <gr0097       Save width of buffer
...
* odd pixel boundaries: do 1st pixel slow, use TFM for the rest
         ...
         ldd   <gr0000         masks for pixels to keep from GP buffer
         IFNE  H6309
         comd
         ELSE
         coma
...
* except for really large PutBlks.  Oh well.
         lda   <gr0001       get end pixel mask
         anda  -1,x         keep only the pixels we want
         ldb   <gr0020+1       inverted mask
         ...
         ENDC
         sta   -1,x         save it
         ldb   <gr0097       get width of the screen
         abx                go to the next line
         dec   <gr0051+1         count down one line
         bne   Put.ATFM
         rts

* Either not even byte, or PSET/LSET or not defaults:use slow PUT
L0DBE    sta   <gr0005           Save # pixels/byte
         pshs  y              Save Get/Put buffer ptr
         ldu   <gr0064           Get PSET vector
         IFNE  H6309
         ldw   <gr0068           Get LSET vector (for PSET routine)
         ELSE
         ldx   <gr0068
         stx   <gr00B5
         ENDC
         ...
         inc   <gr0097           Save # bytes to start of next line GP buffer
...
L0DCB    ldb   <gr0000           Get pixel mask for 1st byte of 1st line in buffr
         lda   <gr004F+1           Get LSB of X size
         beq   L0DED          If 0, just 1 byte to do - use last byte routine
         sta   <gr0099           Save LSB of X size
...
L0DED    lda   ,y+            Get last byte for current line in GP buffer
         ...
         lbsr  L0E70          Yes, go map in next block
L0DFA    ldb   <gr0097         Get # bytes to beginning of next line
         abx                  Point to start of next line
         dec   <gr0051+1         Dec # of lines counter
...
L0E24    subd  <gr003D           Subtract last X coord Gfx cursor was ON at
         std   <gr009B         Save # pixels to end of screen
         ldb   #MaxLine+1     Full height of screen
         subb  <gr003F+1         Calculate # pixels remaining

L0E2F    stb   <gr00A0         Save it
         lbsr  L1EF1          Setup pix mask & shift vector ($79 & $7A)
         lbsr  L0D94          Set up element X&Y sizes (in bytes)
* B=Height of GP buffer (also in <gr0051+1) in bytes
         cmpb  <gr00A0         Compare with room left on window Y axis
         bls   FullSz
         ldb   <gr00A0         Get remaining # lines on window
         stb   <gr0051+1         Save as our single counter of lines left
FullSz   ldd   Grf.LfPx,y     Get # pixels used in 1st byte & last byte
         std   <gr0006         Save them
...
* We now allow GETBlk and PutBlk on text screens, too!
         ...
         stx   <gr0002         Save base of color mask table
...
L0E80    fcb   L0EE0-L0E80    <gr00A3 vector
...
L0E8B    fcb   L0EDF-L0E8B    <gr00A3 vector
...
L0E92    fcb   L0EDD-L0E92    <gr00A3 vector
...
L0E9E    stx   <gr0072         Save get/put screen start address
L0EA0    ldd   <gr009B         ??? x-count to do
         std   <gr009D         ???
         lda   <gr004F+1         Get LSB of X size (in bytes)
         sta   <gr0004         Save # bytes left in width (including partials)
         ldb   <gr0006         Get # of pixels used in 1st byte of GP line
         stb   <gr0097         Save as # pixels left to do in current byte
         ldd   <gr00A5         Get A5 vector
         std   <gr00A1         Save as A1 vector
...
L0EC1    lda   ,y+            Get byte of data from GP buffer
         ...
         ldy   #GrfStrt+L0EE1  Check if <gr00A1 vector points here
         cmpy  <gr00A1         no shifting of bits?
...
* Entry: W=LSET vector (for use with <gr0064,u vector)
...
L0EE1    pshs  cc,d           Save carry & pixel/color masks
         ldd   <gr009D         ??? Get some sort of counter (X width?)
         ...
         anda  <gr0008         Mask out all but common bits of screen/buffer types
...
L0F04    leas  3,s            Eat stack
         lda   <gr0004         Get # bytes wide GP buffer is
         deca                 Decrement it
         beq   L0F20          If totally done buffer width, go to next line
         sta   <gr0004         Save new total
         deca                 If only 1, set up for partially used last byte
         beq   L0F14
         lda   <gr0005         Get # pixels/byte in GP buffer for full byte
         fcb   $8C          skip 2 bytes: same cycle time, 1 byte shorter

L0F14    lda   <gr0007         Get # pixels to do in last (partial) byte of bfr
L0F16    sta   <gr0097         Save # pixels to do in next byte
...
L0F20    ldx   <gr0072         Get screen addr of current line in GP buffer
         ...
         dec   <gr0051+1         Dec # lines left on window / GP buffer
...
fast.set dec   <gr0082+1       account for the first character we printed out
...
* ATD: $83 is unused by anything as far as I can tell.
         sta   <gr0082+1         save count of characters to do for later
...
* perhaps the DEC <gr0082+1 could be here... remove FAST.SET, and fix f1.do

         lda   <Gr.STYMk         is it a text screen?
         bmi   fast.set     yes, make it _really_ fast

         ldb   <gr006E+1       get X size of font
         ...
         lbne  f1.do
         ENDC
         ldx   <gr00AF+1         get cached font pointer
         IFNE  H6309
...
* Note: <gr0061 contains the foreground color mask, <gr0062 contains the background
*   color mask.
...
L106D    pshs  x            save font address
         ...
         ELSE
         lda   <gr000E          no need to preserve regA
         bita  #Prop
         ENDC
...
L10DF    lsl   <gr000B       Shift pixel into carry from font byte
         ...
         lda   <gr0062       Transparent, get bckgrnd color full byte bit mast
...
* Fast horizontal and vertical lines call this after doing a LDW <gr0068 (LSET)
...
L127B    equ   *
         ...
         ldf   <gr0097        Get # bytes wide window is
         clre
...
L127E    tfm   x+,y+         Block move the line
         ...
         clra
         ldb   <gr0097
         std   <gr00B5         regW loaded 
         puls  d 
         subd  <gr00B5         subr w,d
L127E    pshs  d,x,u
         ldb   <gr00B6         get regW
         clra
         tfr   x,u
...
L1291    pshs  y             Save window table ptr
         ...
         decb                0 base
         lda   <gr0070+1        Get Y pixel count
...
L136C    equ   *
         ...
         addd  #1
         ENDC
         std   <gr004F        New X size (in bytes)
         ldd   <gr0070        Get Y pixel count
         std   <gr0051        New Y size (in bytes)
         bra   L13AD
...
* Do the CLS on text screen
         ...
         orb   <gr0062        Mask in background color
         IFNE  H6309
         ...
         ldb   <gr0063        Get #bytes/row
         subb  <gr004F+1        Subtract width twice for char. & attribute
         subb  <gr004F+1        B=# bytes to skip to go to next line
         beq   ClsFTxt       If full width screen, use optomized routine

L13CF    lda   <gr004F+1        Get width of line in chars?
         IFEQ  H6309
         pshs  u
         ldu   <gr00B5
         ENDC
...
ClsFTxt  ldb   <gr004F+1        Get # chars per line
...
* NOTE: <gr0047+1 contains a 0 when coming in here for CLS
*   If this is the only way to get here, may change lda/coma to lda #$ff
* <gr004F=X size in pixels (1-640) to clear
* <gr0051=Y size in pixels (1-200) to clear
...
L13E5    lsrb               move the mask one bit to the right
         ...
         IFNE  H6309
         lde   <gr0051+1         get the lines to clear
         ELSE
         ldb   <gr0051+1
         stb   <gr00B5
         ENDC
...
L13F0    inca               now B=number of pixels per byte (8,4,2);A not B; RG
         ldb   <gr0062              Get backgrnd full-byte pixel mask
...
*         tfr   b,a         don't care about remainder
         ...
         leas  1,s
         ENDC
         cmpb  <gr0063            Get # bytes/row on screen
...
L157B    cmpy  <gr002E         We on current window?
         ...
         ENDC
         bne   L1579          No, exit
         cmpa  <gr0039       get cursor on screen flag
...
* Update txt cursor (on gfx or txt screens) from UPDATE Window 'hidden' call
*L1563    cmpy  <gr002E         We on current window?
*         bne   L1579          No, exit
*         tim   #NoCurs,Wt.BSW,y Cursor on?
*         bne   L1579          No, exit
*         ldb   <gr0039       get GP buffer block number
*         bne   L1579        none, exit
*         lbsr  L1002          Set up font counts
*         bsr   L158B
*         inc   <gr0039       cursor is ON the screen
*L1579    rts   

* Cursor off
*L14F8    oim   #NoCurs,Wt.BSW,y         Set cursor flag to off
* Update text cursor (on gfx or text screens) from within Grfdrv
*L157A    cmpy  <gr002E       We on current window?
*         bne   L158A        No, exit
*         ldb   <gr0039
*         beq   L158A
*         lbsr  L1002        setup font counts
*         bsr   L158B
*         clr   <gr0039       cursor is OFF of the screen
*L158A    rts   
...
* Bring in Get/Put buffer memory bank - put into GRFDRV DAT Img @ gr0087
* Entry: B=MMU block # to get
L017C    clr   <gr0087+2         Make sure System Global is first
         stb   <gr0087+3         Save Block number of Get/Put buffer
...
L16C9    std   <gr0097           Save current color & bit masks
         ...
         ldd   <gr0097           Get color & bit masks back
...
* Next pixel calcs - See if <gr0047 could not be done outside the loop by a
...
* A flag for left/right movement <gr0012 is used.
* Normal line routine
L1724    clr   <gr0012       flag for X swap
         ldd   <gr004B       current X
         subd  <gr0047       new X
         std   <gr0012+1       save dX
         bpl   L1734
         ...
         sbca  #0
         ENDC
         std   <gr0012+1       force dX>0
L1734    ldb   <gr0063       BPL bytes/line
         clra  
         std   <gr0012+5       save 16-bit bytes per line
         ldd   <gr004D       current Y
         subd  <gr0049       subtract working Y
         std   <gr0012+3       save dY
         ...
         sbca  #0
         ENDC
         std   <gr0012+3       force dY>0
         ldd   <gr0012+5       up/down movement; up=+ down=-
         IFNE  H6309
         negd
         ELSE
         nega
         ...
         sbca  #0
         ENDC
         std   <gr0012+5       now points the correct direction

L1753    ldd    <gr0012+1      compare dX with dY to find larger
         cmpd   <gr0012+3
         bcs    Ylarge
         IFNE   H6309
         asrd               error = dX/2
...
L1788b   ldd   <gr0075       get error
         bpl   L177D        if >=0
         addd  <gr0012+1       add in dX
         std   <gr0075       save new working error
         ldd   <gr0012+5       get BPL
         IFNE  H6309
         addr  d,x
...
L177D    subd  <gr0012+3       take out one BPL
...
L1856    bsr   L1884          Make sure window is graphics
         ldd   <gr0053           Get radius (horizontal)
         IFNE  H6309
...
L18D4    lbcs  L1A75          Nope, exit with error
         ...
         std   <gr001E           Move to working area
...
* (once for top line, once for bottom line)
         ...
         IFNE  H6309
         ldq   <gr00AD         Get original X coords back
         std   <gr0047         Save Start X
         stw   <gr004B         Save End X
         ELSE
         ldd   <gr00AF
         std   <gr00B5
         std   <gr004B
...
* NOTE: THIS WILL MODIFY <gr0047 AS IT GOES THROUGH THE LINE!
...
SetX     addd  <gr0018         Add X center point
...
SetY     addd  <gr001A         Add Y center point
...
* the additional benefit is that <gr0028 is now the full-byte color mask
...
L1C93    ldb   ,s+          grab y-direction to travel
         ...
         stb   <gr002b       save direction to travel in
...
L1CB7    clrb                 Clear carry as default (no error)
         ldb   <gr002a         Get done/error flag
...
* <gr0028 = full-byte color mask to paint on
* <gr0029 = pixels per byte
...
FFILL.2  ldu   <gr0047
         ...
         clr   <gr002C         clear flag: no pixels done yet
...
DoChecks ldd   <gr00AF         Get Y value from 1st FFill line
         ...
         cmpu  <gr00AD         right side X lower or same as original X?
         bhi   Not1st       No, draw it
         ldd   <gr0020         Get left X coord
         cmpd  <gr00AD         left side X higher or same as original X?
         blo   Not1st       No, draw it
         leas  4,s          We already did this, eat stack & exit w/o error
         clrb
         jmp   >GrfStrt+SysRet

Not1st   ldd   <gr004B         get old coordinate: U=<gr0047 already
         pshs  d,x,y,u
         stu   <gr004B         save as X-end
         ldd   <gr0020         get LHS X coordinate
...
L1D05    puls  u            restore PC of calling routine
         ...
         IFNE  H6309
         ldw   <gr0047         Get 'working' X coord
         ELSE
         ldd   <gr0047
         std   <gr00B5
...
* ATD: mod: <gr0028 is full-byte color mask
...
* <gr0047=start X, <gr0049=current Y,  <gr004B=end X
...
* at least 0.
         ...
         lda   <gr002b       Get save current Y-direction
...
L1D98    lbsr  FFILL.2
         ...
         lda   <gr002b       grab direction flag
...
L1DC4    cmps  <gr003B         Stack about to get too big?
         bhi   L1DCB          No, continue
         clr   <gr002a         Yes, set flag to indicate stack overflow
...
L1DEE    cmps  <gr003B       check against lowest possible stack
         bhi   L1DF5        Question:  Why not just an in-line check?
         clr   <gr002a       clear flag: stack is too low
L1DF5    rts   
...
L1EF1    lda   <Gr.STYMk       get screen type
         ...
         sta   <gr0079       Preserve mask
         abx                Point to bit shift routine
         stx   <gr0077       Preserve vector to bit shift routine
         rts   
